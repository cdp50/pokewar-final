...
https: //pokeapi.co/api/v2/pokemon/{id or name}/
  ...
  // appendNewElement("img", "alt", "place holder pokemon", ".one", null, "src", )
...
async function renderPokeModal(data) {
  ...
  const formHTML = `
    <form action="/poke/addToTeam/" method="post">
      ...
      <input type="hidden" name="pokemon_name" ${data.name ? `value="${data.name}"` : ""}>

      <input type="hidden" name="pokemon_type" ${data.types ? `value="${data.types[0].type.name}"` : ""}>
